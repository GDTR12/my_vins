...
class FrontEnd(Node):
    ...
    def createMatchPrevShow(self, img0, kpts0, img1, kpts1, matches0, matches1):
        img = cv2.hconcat([img0, img1])
        w = img0.shape[1]
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        kpts0 = torch.round(kpts0).int().cpu().squeeze()
        kpts1 = torch.round(kpts1).int().cpu().squeeze()
        k0 = torch.split(kpts0, 1, dim=0)
        k1 = torch.split(kpts1, 1, dim=0)
        # self.get_logger().info(f'{k0[0].shape}')
        for idx, m in enumerate(torch.split(matches0, 1, dim=1)):
            m = m.reshape((1))
            if m == -1:
                continue
            else:
                # self.get_logger().info(f'{k0[idx][0,0].item(), k0[idx][0,1].item()}')
                cv2.circle(img, (k0[idx][0,0].item(), k0[idx][0,1].item()), 2, (0,0,255))
                cv2.circle(img, (k1[m][0,0].item() + w, k1[m][0,1].item()), 2, (0,0,255))
                cv2.line(img, 
                         (k0[idx][0,0].item(), k0[idx][0,1].item()),
                         (k1[m][0,0].item() + w, k1[m][0,1].item()),
                         (0,255,0),
                         1)
